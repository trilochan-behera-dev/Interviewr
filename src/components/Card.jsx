export default function Card({ data, onClick, key }) {
  const color =
    {
      Remote: "bg-green-800",
      Onsite: "bg-blue-800",
  ...
  return (
    <>
      <div
        className="bg-white border-2 border-gray-200 shadow-sm rounded-lg text-gray-700 font-normal p-4 w-full text-sm"
        key={key}
      >
        <div className="flex justify-between pb-1 gap-4">
          <div>
            ...
            <div className="font-medium">
              Date-{new Date(data.applied_date).toLocaleDateString("en-GB")}
            </div>
          </div>
          <div
